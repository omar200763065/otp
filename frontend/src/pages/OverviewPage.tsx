...
import { MessageSquare, CheckCircle, AlertTriangle, Key, Activity, Database, Cpu, Zap, ShieldCheck, ArrowUpRight, Inbox } from 'lucide-react';
...
export const OverviewPage: React.FC = () => {
  ...
  const chartData = data?.chartData || [
    { time: '00:00', sent: 0, verified: 0 },
    { time: '04:00', sent: 0, verified: 0 },
    { time: '08:00', sent: 0, verified: 0 },
    { time: '12:00', sent: 0, verified: 0 },
    { time: '16:00', sent: 0, verified: 0 },
    { time: '20:00', sent: 0, verified: 0 },
    { time: '23:59', sent: 0, verified: 0 },
  ];
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3.5 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 800, background: 'linear-gradient(90deg, #f8fafc 0%, #2dd4bf 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            نظرة عامة وإحصائيات المنصة
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            تتبع حركات ومحاولات التحقق الخاصة بتطبيقاتك ورقم الواتساب الخاص بك
          </Typography>
        </Box>
      ...
      <Grid container spacing={2.5}>
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 3, borderRadius: 4, display: 'flex', alignItems: 'center', gap: 2.5, borderLeft: '4px solid #06b6d4' }}>
          ...
          <Paper sx={{ p: 3.5, borderRadius: 4, height: 400 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 800, display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                حركة الإرسال المباشرة
              </Typography>
              <Chip label="تحديث تلقائي" size="small" sx={{ background: 'rgba(45, 212, 191, 0.1)', color: '#2dd4bf', fontWeight: 700 }} />
            </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Box sx={{ p: 2, borderRadius: 3, bgcolor: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  ...
                  <Box>
                    <Typography variant="body2" sx={{ fontWeight: 700 }}>قاعدة البيانات المحفوظة</Typography>
                    <Typography variant="caption" color="text.secondary">حفظ البيانات والمستخدمين</Typography>
                  </Box>
                </Box>
              ...
              <Box sx={{ p: 2, borderRadius: 3, bgcolor: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  ...
                  <Box>
                    <Typography variant="body2" sx={{ fontWeight: 700 }}>ذاكرة الكاش والتأمين</Typography>
                    <Typography variant="caption" color="text.secondary">منع التكرار والحماية</Typography>
                  </Box>
                </Box>
                <Chip label="جاهز" color="success" size="small" sx={{ fontWeight: 700, borderRadius: 2 }} />
              </Box>
            </Box>

            <Box sx={{ pt: 1 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.8 }}>
                <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary' }}>مؤشر أمان النظام (Security Status)</Typography>
                <Typography variant="caption" sx={{ fontWeight: 800, color: '#2dd4bf' }}>100% المؤمّن</Typography>
              </Box>
              ...
              {recentList.length === 0 ? (
                  ...
                  <TableCell colSpan={6} align="center" sx={{ py: 6 }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1.5, color: 'text.secondary' }}>
                      ...
                      <Typography variant="body1" sx={{ fontWeight: 700, color: '#94a3b8' }}>
                        لا توجد أي عمليات إرسال مسجلة حتى الآن.
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        قم بإرسال كود تجريبي من مختبر الـ API لتشاهد العمليات تظهر هنا مباشرة.
                      </Typography>
                    </Box>
                  </TableCell>
                </TableRow>
                ...
                recentList.map((tx: any) => (
                  <TableRow key={tx.id} hover>
                    <TableCell sx={{ fontWeight: 700 }}>{tx.appName || tx.app?.name || 'التطبيق الخاص بك'}</TableCell>
